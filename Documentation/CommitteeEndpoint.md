  ...
  - The unique ID value for the committee. Parent committee `<systemCode>` values end in a double zero while those for a subcommittee end with numerals other than a double zero.
    ...
    - `<reports>`
      - Indicator of whether there are reports issued by the committee.    
        - `<url>` (e.g., https://api.data.gov/congress/v3/committee/house/hspw00/reports?format=xml)
          - A referrer URL to the committee's reports in the API. 
        - `<count>` (e.g., 1373)
    ...
    - `<bills>`
      ...
      - `<url>` (e.g., https://api.data.gov/congress/v3/committee/house/hspw00/bills?format=xml)
        - Referrer URL to the committee's bills in the API. 
      - `<count>` (e.g., 25313)
...
Parent container for reports issued by a committee. The `<reports>` element may contain the following children, which are repeatable:
 - `<item>`
   ...
   - `<url>` (e.g., https://api.data.gov/congress/v3/committeeReport/109/HRPT/570?format=xml)
     - A referrer URL to the committee report level in the API. 
...
Parent container for bills and resolutions associated with the committee or subcommittee. The `<committee-bills>` element may contain the following children:
 ...
 - `<bills>`
   - Container for all bills and resolutions associated with the committee. The `<bill>` element is repeatable and may contain the following children:
   - `<bill>`
