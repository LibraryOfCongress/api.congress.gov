...
- `<item>`
  - Container for an individual legislative measure sponsored by the member. The `<item>` element may include the following children:
   ...
   - `<policyArea>`
    - Container for the policy area term of the bill or resolution. Every bill and resolution is assigned one policy area term out of a list of 32; view a field values list of policy area terms on Congress.gov. Read more about policy area terms on Congress.gov.
    - A `<policyArea>` element may include the following children:
     - `<name>` (e.g. Commerce)
      - The policy area term assigned to the bill or resolution by CRS.
   - `<latestAction>`
    - Parent container for the latest action taken on the bill or resolution. A `<latestAction>` element may include the following children:
